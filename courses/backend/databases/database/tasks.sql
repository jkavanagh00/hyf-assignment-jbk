...
INSERT INTO task (title, description, created, updated, due_date, status_id) VALUES
  ('Study SQL Queries', 'Practice writing SQL queries for data retrieval', datetime('now'), datetime('now'), '2025-08-02', 3),
  ('Learn Database Design', 'Study ER modeling and normalization', datetime('now'), datetime('now'), '2025-08-10', 1),
  ('Write Unit Tests', 'Add test coverage for user authentication', datetime('now'), datetime('now'), '2025-08-05', 1),
  ('Deploy Application', 'Set up production environment', datetime('now'), datetime('now'), '2025-08-20', 1);
...
UPDATE task
	SET due_date = '2026-04-27 00:19:08'
	WHERE id = 40;

-- Part 1, Question 5: Change task status to 'done'
UPDATE task
	SET status_id = 3
	WHERE id = 40;
...
-- Assignment, Part 2: Working with Relationships

-- Part 2, Question 1: List all users who don't have any tasks assigned
SELECT name 
    FROM user
    WHERE id NOT IN (SELECT user_id FROM user_task);
...
-- Part 3, Question 1: Add the priority column to the task table and constrain possible values
ALTER TABLE task
	ADD COLUMN priority TEXT NOT NULL DEFAULT 'medium'
	CHECK (priority IN ('low', 'medium', 'high'));

-- Part 3, Question 2: Create a mix of different priority tasks
UPDATE task
	SET priority = 'low'
	WHERE id > 20;
UPDATE task
...
-- Part 3, Question 3: Create a new table called category
...
-- Part 3, Question 4: Create a linking table task_category
...
-- Part 3, Question 5: Insert categories
INSERT INTO category (name, color) VALUES ('work', 'blue');
INSERT INTO category (name, color) VALUES ('personal', 'green');
INSERT INTO category (name, color) VALUES ('study', 'red');
INSERT INTO category (name, color) VALUES ('home', 'orange');
INSERT INTO category (name, color) VALUES ('hobby', 'purple');
INSERT INTO category (name, color) VALUES ('fun', 'yellow');

-- Part 3, Question 6: Assign categories to tasks
...
INSERT INTO task_category (task_id, category_id) VALUES (5, 4);
INSERT INTO task_category (task_id, category_id) VALUES (6, 2);
INSERT INTO task_category (task_id, category_id) VALUES (7, 1);

-- Assignment, Part 4: Advanced Queries

-- Part 4, Question 1: Find all tasks in a specific category (work, id 1)
SELECT *
...
-- Part 4, Question 2: List tasks ordered by priority and due date
SELECT *
FROM task
ORDER BY
  CASE priority
...
SELECT * FROM task_category tc;

-- Part 4, Question 3: Find the category with the most tasks
SELECT
...
-- Part 4, Question 4: Get all high priority tasks that are either 'in progress' or 'not started'
-- note: status labels are not consistent with those in exercise
SELECT *
	FROM task t
	WHERE priority = 'high'
...
ORDER BY category_count DESC, u.name;